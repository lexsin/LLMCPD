...
def main(
    ...
    output=OUTPUT,
    database=None,
    reported_table="第二批上报算力",
    result_suffix="_llm_最终版.xlsx",
):
    contains = None
    interval_count = 0
    if database:
    ...
    for directory in sorted(path for path in root.iterdir() if path.is_dir()):
        result = directory / f"{directory.name}{result_suffix}"
    ...
    summary.append(["企业名称", "GPU可能性高", "GPU可能性中", "其他结果", "结果合计"])
    ...
    for enterprise, result in sources:
        ...
        is_first_table = True
        for source_sheet in source_workbook.worksheets:
            if category_for(source_sheet.title) == "低":
                continue
    ...
    for cell in summary[summary.max_row]:
        cell.font = Font(bold=True)
    for column, width in {"A": 22, "B": 16, "C": 16, "D": 14, "E": 14}.items():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="汇总已完成企业的高中等级 LLM 结果")
    parser.add_argument("--root", type=Path, default=ROOT)
    parser.add_argument("--output", type=Path)
    parser.add_argument("--database", type=Path)
    parser.add_argument("--reported-table", default="第二批上报算力")
    parser.add_argument(
        "--result-suffix", default="_llm_最终版.xlsx",
        help="per-enterprise workbook suffix, e.g. _llm_最终版_主动探测.xlsx",
    )
    args = parser.parse_args()
    main(
        args.root,
        args.output or args.root / "已完成企业结果汇总.xlsx",
        args.database,
        args.reported_table,
        args.result_suffix,
    )
