...
RUN_ID=rescan_237_on_343_source_policy_v5_fullport_20260911
...
mapfile -t ALL_LOCATIONS < <(
  sqlite3 "$ROOT/data/llm_detect.sqlite3"     "select distinct location from source_ip_ranges order by location"
)
...
[ "${#ALL_LOCATIONS[@]}" -eq 41 ] || {
  echo "Expected 41 deduplicated enterprises, got ${#ALL_LOCATIONS[@]}" >&2
  exit 1
}
[ "${#LOCATIONS[@]}" -eq 36 ] || {
  echo "Expected 36 enterprises to rerun on 3.43, got ${#LOCATIONS[@]}" >&2
  exit 1
}
...
run_one() {
  ...
  if [ ! -s "$input" ]; then
    python3 "$ROOT/ip_source_db.py" export       --location "$location" --output "$input" || return $?
  fi
...
log "Rerun started on $(hostname): total=${#LOCATIONS[@]}"
...
log "Rerun finished: total=${#LOCATIONS[@]} complete=$complete failed=$failed"
