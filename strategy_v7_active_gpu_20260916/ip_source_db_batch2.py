#!/usr/bin/env python3
# Separate importer for the second base-IP batch. It never reads or writes first-batch rows.
# -*- coding: utf-8 -*-
import argparse
import csv
import hashlib
...
DATA = ROOT / "data"
BASE_DATA = DATA / "base_2_20260910"
...
def schema(connection):
    connection.executescript(
        ...
        CREATE TABLE IF NOT EXISTS import_runs_batch2 (
        ...
        CREATE TABLE IF NOT EXISTS source_ip_ranges_batch2 (
            id INTEGER PRIMARY KEY,
            batch_no INTEGER NOT NULL DEFAULT 2,
        ...
        CREATE INDEX IF NOT EXISTS idx_ranges_batch2_location ON source_ip_ranges_batch2(location);
        CREATE INDEX IF NOT EXISTS idx_ranges_batch2_bounds ON source_ip_ranges_batch2(start_ip_int, end_ip_int);
        """
    )
...
def import_sources(connection, sources, replace_all):
    ...
    cursor = connection.execute(
        "INSERT INTO import_runs_batch2(started_at, source_count) VALUES (?, ?)",
        (started, len(sources)),
    )
    ...
    if replace_all:
        connection.execute("DELETE FROM source_ip_ranges_batch2")
    ...
    for location, path in sources:
        ...
        connection.execute(
            "DELETE FROM source_ip_ranges_batch2 WHERE source_path = ?",
            (str(path),),
        )
        ...
        for sheet, source_row, values, start_column, end_column, unit_column in reader:
            ...
            connection.execute(
                """
                INSERT OR IGNORE INTO source_ip_ranges_batch2 (
    ...
    connection.execute(
        "UPDATE import_runs_batch2 SET completed_at = ?, valid_count = ?, invalid_count = ? WHERE id = ?",
