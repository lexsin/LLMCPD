...
phase0:
  non_http_prefixes:
    ...
    - "RFB "              # VNC RFB 协议握手
    - "\xff\xfd"          # Telnet IAC DO 协商
    - "\x4a\x00\x00\x00"  # MySQL 初始握手（包头长度 J + NUL×3）
    - "N\x00\x00\x00"     # PostgreSQL 启动消息长度前缀
```
...
> **注意：** 若原始响应以 `HTTP/` 开头（合法 HTTP 响应，可能携带 gzip 压缩 body），跳过二进制检测，直接进入后续流程。这防止压缩响应被误判为二进制协议。
...
phase1:
  probe_paths:
    - /v1/models         # 优先级 1：OpenAI 兼容 API（标准路径）
    - /api/v1/models     # 优先级 2：反向代理前缀变体，别名为 /v1/models
    - /openai/v1/models  # 优先级 3：另一种代理前缀，别名为 /v1/models
    - /api/tags          # 优先级 4：Ollama
    - /api/version       # 优先级 5：Ollama 版本
    - /info              # 优先级 6：TGI
    - /health            # 优先级 7：探测但无确认规则（见下）
    - /docs              # 优先级 8：Swagger/OpenAPI 文档
    - /v2/models         # 优先级 9：Triton/KServe v2 推理 API
    - /                  # 最后：检查前端页面特征
```
...
```yaml
  confirm_rules:
    ...
    # 反向代理前缀变体：使用 cache_also_as 别名，无需修改 Phase 2
    - path: /api/v1/models
      when_status: 200
      cache_also_as: /v1/models    # 响应同时写入此路径的缓存
      match:
        type: json_all
        rules:
...
**`cache_also_as`（可选）：** 当路径命中确认规则后，将该响应额外写入指定的别名路径缓存。用于代理前缀变体路径，使 Phase 2 中依赖 `requires_cached_ok: /v1/models` 的工具无需修改。例如 `/api/v1/models` 命中后写入 `/v1/models`，llama.cpp、fastchat 等工具照常工作。
  ...
  suspect:
    ...
    extra_predicates:            # 额外复合条件（满足任一即可）
      - type: body_all_substrings_ci
        ...
        exclude_if_contains:     # 同时出现以下词则该条件不成立
          - "copyright"
          - "terms of service"
          - "privacy policy"
```
...
**`exclude_if_contains`（仅 `body_all_substrings_ci` 支持）：** 若 body 中包含排除词列表中的任一词（大小写不敏感），该条件视为不成立，即使 `values` 全部出现也不会触发疑似。用于过滤包含"chat"和"model"字样的营销页面（如版权声明、服务条款页）。
  ...
  guards:
    - description: "frontend UI overrides weak API-only signal"
      ...
      downgrade_for_paths:            # 对这些路径的确认结果执行降级
        - /api/version
        - /v1/models    # 反向代理前端（如 open-webui 挂载 /v1/models）也应降级
```

**执行时机：** 某个 confirm_rule 命中后立即检查所有 guards。若当前命中路径在 `downgrade_for_paths` 中，且根页面包含 `root_contains_any` 中的任一字符串，则将 `确认` 降级为 `疑似`。

### 3.5 auth_suspect — 认证阻断疑似规则

当 LLM API 配置了鉴权（如需要 API Key），所有探测路径都会返回 401/403，正常确认规则无法触发。`auth_suspect` 通过检测鉴权响应体中的 LLM 相关关键词，将这类目标标记为 `疑似`：

```yaml
  auth_suspect:
    status_codes: [401, 403]       # 触发鉴权检查的状态码
    body_keywords:                 # 响应 body 中含任一词则视为 LLM 鉴权
      - bearer
      - api-key
      - api_key
      - openai
...
**执行逻辑：**
1. 在 Phase 1 探测循环中，若某路径返回 401/403 且 body 含任一关键词，记录 `has_auth_signal = True`
2. 整个探测循环结束后，若 `is_llm` 仍为 `"否"` 且 `has_auth_signal`，则将 `is_llm` 设为 `"疑似"`
3. 若任一路径已经产生了更强的信号（`确认` 或 `疑似`），此规则不生效

**可定制：**
- 删除 `auth_suspect` 或将其设为 `null` 可完全关闭此功能
- 在 `body_keywords` 中添加具体框架名（如 `"ollama"`）可提升精度
...
| `version_from` | 否 | 主匹配**或补充匹配**成功后额外提取版本号，写入 `deploy_version` 列 |
| `supplements` | 否 | 主匹配失败时按序尝试的补充探测列表；每条有独立 `source` 和匹配逻辑；成功后也会执行 `version_from` |
...
> **与 `cache_also_as` 的联动：** 若 Phase 1 通过 `/api/v1/models` 确认（`cache_also_as: /v1/models`），Phase 2 中所有 `requires_cached_ok: /v1/models` 的工具均可正常触发，无需任何改动。

### 4.4 添加新部署工具

在工具列表中插入一条记录，选择合适位置（即优先级）。以下为已内置工具，覆盖主流框架：

| 工具 | 主要探测路径 | 核心关键词 |
|------|------------|----------|
| open-webui | `root` | `open webui` / `open-webui` |
| one-api | `root` | `One API` / `New API` |
| ollama | `/api/tags`（缓存+GET） | `models` 键 |
| tgi | `/info` | `model_id` + `max_input_length` |
| vllm | `/v1/models` → `/metrics` | `owned_by=vllm` / `vllm_` |
| llama.cpp | `/props` | `default_generation_settings` |
| sglang | `/get_server_info` | `sglang` |
| litellm | `/model/info` | `litellm_params` |
| fastchat | `/docs` | `fastchat` |
| xinference | `/docs` | `xinference` |
| localai | `/docs` | `localai` |
| triton | `/v2/models` | `models` 键（v2 API） |
  ...
  post_probes:
    - path: /v1/chat/completions
      ...
      on_error:                  # 错误响应中往往包含可用模型列表
        ...
        extract:
          type: error_regex
          regex: '"([a-zA-Z0-9_\-./: ]+)"'   # 宽泛正则捕获组（回退使用）
          context_patterns:                   # 精确正则，优先于宽泛 regex
            - 'model["\s:=]+([a-zA-Z0-9_\-./]+)'
            - "model '([a-zA-Z0-9_\\-./]+)'"
          exclude_patterns:                   # 黑名单词：匹配但排除
            - test
            - "your-model-id"
            - string
            - "null"
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `regex` | 是 | 宽泛正则捕获组，当 `context_patterns` 无结果时使用 |
| `context_patterns` | 否 | 精确正则列表（先尝试）；产生结果则不再使用 `regex`；适合从错误消息中精确提取模型名 |
| `exclude_patterns` | 否 | 黑名单字符串列表（大小写不敏感精确匹配）；过滤掉已知的占位符或噪声词 |
| `filter.min_len` | 否 | 捕获结果最小长度（不含边界，即 `>`） |
| `filter.max_len` | 否 | 捕获结果最大长度（不含边界，即 `<`） |
| `filter.must_contain_any_char` | 否 | 捕获结果必须包含列表中至少一个字符 |

**提取优先级：** `context_patterns`（精确）→ `regex`（宽泛）→ 均无结果则尝试下一个 `post_probe`。
...
| `body_all_substrings_ci` | `values: [str]`，可选 `exclude_if_contains: [str]` | 所有 value 均不区分大小写出现在 body 中；若 `exclude_if_contains` 中任一词出现则整体返回 false |
...
| `error_regex` | `regex: str`，可选 `context_patterns: [str]`、`exclude_patterns: [str]`、`filter?: {}` | 精确正则优先，宽泛 regex 回退；filter + exclude_patterns 过滤结果 |
...
### 场景 2：添加新 LLM 框架指纹（以自定义框架为例）

Triton、SGLang、LiteLLM、OneAPI 等主流框架已内置。如需添加自定义框架：

1. 在 `phase1.probe_paths` 加入特征路径（如 `/custom/info`）
2. 在 `phase1.confirm_rules` 添加对应确认规则
3. 在 `phase2.tools` 插入识别条目
...
phase2:
  tools:
    - name: my-framework
      scope: [confirmed]
      requires_cached_ok: /custom/info
      match:
        source: "cached:/custom/info"
        type: body_substring_ci
        value: "my-framework"
```
...
### 场景 4：新增 Phase 0 协议排除

MySQL、PostgreSQL、VNC、Telnet 前缀已内置。如需新增其他协议：
...
phase0:
  non_http_prefixes:
    - "-ERR"
    # ... 原有前缀 ...
    - "AMQP"      # RabbitMQ/AMQP 握手
```

> 注意：YAML 字符串自动 UTF-8 encode，引擎会将其作为字节前缀比较。含 NUL 等不可见字节时用转义序列 `\x00`。

### 场景 5：为鉴权 LLM API 启用认证阻断检测

若扫描目标包含配置了 API Key 的 LLM 服务，使用 `auth_suspect` 避免因 401/403 被误判为非 LLM：
...
phase1:
  auth_suspect:
    ...
    body_keywords:
      ...
      - authorization
      - llm
      - "x-api-key"    # 添加自定义鉴权头关键词
```

若不需要此功能，设置 `auth_suspect: null` 或删除该字段。
...
scan_config.py            ← 加载器 + 匹配引擎
  ...
  ├── eval_match()        ← 12 种匹配算子（含 exclude_if_contains）
  └── extract_models()    ← 3 种提取算子（含 context_patterns / exclude_patterns）
        │
        ▼  import
